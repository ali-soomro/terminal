...
void RegExpFilter::process()
{
    ...
    // otherwise the iterator below will loop indefinitely
    if ( _searchText.match(QString()).hasMatch() )
        return;

    auto matchIter = _searchText.globalMatch(*text);
    while (matchIter.hasNext())
    {
        QRegularExpressionMatch m = matchIter.next();
        ...
        getLineColumn(pos + len, endLine, endColumn);

        RegExpFilter::HotSpot* spot = newHotSpot(startLine, startColumn,
                                       endLine, endColumn);
        spot->setCapturedTexts(m.capturedTexts());
