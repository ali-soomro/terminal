...
ColorEntry ColorScheme::colorEntry(int index , uint randomSeed) const
{
        ...
        !_randomTable[index].isNull() )
    {
        ...
        int hueDifference = range.hue ? (int(QRandomGenerator::global()->bounded(range.hue)) - range.hue/2) : 0;
        int saturationDifference = range.saturation ? (int(QRandomGenerator::global()->bounded(range.saturation)) - range.saturation/2) : 0;
        int  valueDifference = range.value ? (int(QRandomGenerator::global()->bounded(range.value)) - range.value/2) : 0;
